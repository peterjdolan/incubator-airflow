...
class CloudMLModelOperator(BaseOperator):
    ...
    :param model_name: The name of the Cloud ML model. If set, then the 'model'
        parameter will be populated with only the model name.
    :type model_Name: string
    ...
    def execute(self, context):
        ...
        if 'name' not in self._model:
            logging.info('Populating model with name')
...
class CloudMLVersionOperator(BaseOperator):
    ...
    :param version_name: The `name` of the version, which will be set in the
        `version` dictionary if provided.
    :type version_name: string
    ...
    def execute(self, context):
        if 'name' not in self._version:
            logging.info('Setting verison name in version')
